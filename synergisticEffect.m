%---------------------------------------------------------------------------
%% effectOfInterferonDosagewithPC.m
...
%SECTION TO CREATE JUST INTERFERON GRAPH
%***************************************************************************
...
subplot(2,1,1);
plot(time,((xa(:,3)))','--','color',colors_p(i,:),'LineWidth',2);
hold on
end
...
title('Prion Concentration Over Time at Differing Dosages of Interferons and Absent Pharmacological Chaperones','Interpreter','latex','FontSize',30);
ax = gca;
ax.FontSize = 20; 
%%declaration/formatting of color bar
colormap(colors_p);
c = colorbar('Ticks',[0 1],'TickLabels',[I_samples(1) I_samples(length(I_samples))]);
c.Label.String = 'Interferon Dose (nM/day)'
%---------------------------------------------------------------------------

%plot P state variable equilbrium solution vector against time
subplot(2,1,2);
plot(I_samples,final_P,'--','color',[1,0,0],'LineWidth',4);
...
%SECTION TO CREATE INTERFERON AND PC GRAPH
%***************************************************************************
%%update parameter values (subject to change)
%---------------------------------------------------------------------------
I = 0;
mu_P = 0.047;
...
color1 = [0, 0, 1];
%%intialize ending color of gradients
color2 = [211,107,255]/255;
%%interpolate all colors inbetween start and end color
colors_p = [linspace(color1(1),color2(1),lengthColor)', linspace(color1(2),color2(2),lengthColor)', linspace(color1(3),color2(3),lengthColor)'];
%---------------------------------------------------------------------------

%%Solve the system of differentials for every value of I sample
for i=1:length(I_samples)
    ...
    %%plot P against time for given value of I
    subplot(2,1,1);
    plot(time,((xa(:,3)))','color',colors_p(i,:),'LineWidth',4);
    hold on
end
...
axis tight
set(groot,'defaultAxesTickLabelInterpreter','latex'); 
xlabel('Time (day)','Interpreter','latex','FontSize',20)
ylabel('Prion Concentration (nM)','Interpreter','latex','FontSize',20)
title('\textbf{Synergistic Effect of Combined Treatment}','Interpreter','latex','FontSize',60);
ax = gca;
ax.FontSize = 20; 
%%declaration/formatting of color bar
colormap(colors_p);
c = colorbar('Ticks',[0 1],'TickLabels',[I_samples(1) I_samples(length(I_samples))],'TickLabelInterpreter','latex','FontSize',20);
c.Label.String = 'Interferon Dose (nM/day)'
...
%plot P state variable equilbrium solution vector against time
subplot(2,1,2);
plot(I_samples,final_P,'color',[0,0,1],'LineWidth',4);
...
ylabel('Final Prion Concentration (nM)','Interpreter','latex','FontSize',20)
axis tight
legend({'Pharmacological Chaperones Absent','Constant Pharmacological Chaperones'},'Interpreter','latex','FontSize',20)
ax = gca;
ax.FontSize = 20; 
%---------------------------------------------------------------------------
%***************************************************************************
